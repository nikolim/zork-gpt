...
def create_chat():
    ...
    print("Creating chat with original id: {}".format(original_id))
    history = deepcopy(saved_chats.get(original_id)["messages"] if original_id else [])
    chatbot = get_model(model_name)(
        history=history,
        original_id=original_id,
        name=saved_chats.get(original_id, {}).get("name", ""),
    )
    current_id = str(uuid.uuid4())
    chatbots[current_id] = chatbot
    ...
    if original_id:
        ...
        print("New chat created with id: {}".format(current_id))
        return jsonify(
            {
                "current_id": current_id,
                "messages": chatbot.history,
